...
int main(void)
{
    ...
    if (!ext_supported(exts, "EGL_EXT_gl_colorspace_display_p3")) {
        fprintf(stderr, "EGL_EXT_gl_colorspace_display_p3 not supported\n");
        eglTerminate(dpy);
        return 1;
    }
    printf("EGL_EXT_gl_colorspace_display_p3: supported\n");
    ...
    wc.lpszClassName = "EGLDisplayP3";
    RegisterClassEx(&wc);

    RECT rect = { 0, 0, WIDTH, HEIGHT };
    AdjustWindowRect(&rect, WS_OVERLAPPEDWINDOW, FALSE);
    HWND hwnd = CreateWindowEx(0, "EGLDisplayP3", "EGL Display P3",
    ...
    if (surf == EGL_NO_SURFACE) {
        fprintf(stderr, "EGL_EXT_gl_colorspace_display_p3 not supported (eglCreateWindowSurface failed: 0x%x)\n", eglGetError());
        DestroyWindow(hwnd);
        eglTerminate(dpy);
        return 1;
    }
