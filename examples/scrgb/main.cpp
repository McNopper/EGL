...
int main(void)
{
    ...
    if (!ext_supported(exts, "EGL_EXT_gl_colorspace_scrgb")) {
        fprintf(stderr, "EGL_EXT_gl_colorspace_scrgb not supported\n");
        eglTerminate(dpy);
        return 1;
    }
    printf("EGL_EXT_gl_colorspace_scrgb: supported\n");
    ...
    wc.lpszClassName = "EGLscRGB";
    RegisterClassEx(&wc);

    RECT rect = { 0, 0, WIDTH, HEIGHT };
    AdjustWindowRect(&rect, WS_OVERLAPPEDWINDOW, FALSE);
    HWND hwnd = CreateWindowEx(0, "EGLscRGB", "EGL scRGB (HDR)",
    ...
    if (surf == EGL_NO_SURFACE) {
        fprintf(stderr, "EGL_EXT_gl_colorspace_scrgb not supported (eglCreateWindowSurface failed: 0x%x)\n", eglGetError());
        DestroyWindow(hwnd);
        eglTerminate(dpy);
        return 1;
    }
