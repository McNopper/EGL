...
EGLBoolean angle_init(EGLint* es_max_supported)
{
    ...
    if (!g.libEGL || !g.libGLESv2)
    {
        ...
        if (g.libEGL)    { FreeLibrary(g.libEGL);    g.libEGL = nullptr; }
        if (g.libGLESv2) { FreeLibrary(g.libGLESv2); g.libGLESv2 = nullptr; }
        return EGL_FALSE;
    }
...
{
    if (!g.ready || !out_surface) return EGL_FALSE;
...
{
    if (!g.ready || !out_context) return EGL_FALSE;
    if (major < 2) major = 2;
...
{
    if (!g.ready || !surface) return EGL_FALSE;
...
{
    if (!g.ready || !context) return EGL_FALSE;
